    ...
    for template_key in template.keys():  # For every key:value pair defined in the template
        ...
        if template_value == "isnumeric":
            try:
                ...
                float(cue.get_command().get(template_key))
            except ValueError:
        ...
        # otherwise check if the template value and cue value are the same
            ...
            if template_key in cue.get_command():
                #if cue["command"][template_key] != template_value:
                if cue.get_command().get(template_key) != template_value:
                    return False
...
def timing_deviation(cue_start, cue_end, tolerance, b_d_max, b_d_min):
    ...
    break_duration = cue_start.get_command().get("break_duration")
    cue_start_time = cue_start.get_command().get("pts_time")
    cue_end_time = cue_end.get_command().get("pts_time")
...
def verifytriggers(body_log, config):
    ...
    if break_start_exist:  # Make sure template has at least break start to continue
        ...
        for pair in pairs:
            ...
            cue = Cue(pair["input_trigger"].split(":")[3])
            ...
            if cue.get_info_section().get("splice_command_type") == break_start_template["splice_command_type"]:
        ...
        if len(break_start_pairs) == len(break_end_pairs):
            for i in range(len(break_start_pairs)):
                ...
                start_cue = Cue(break_start_pairs[i]["input_trigger"].split(":")[3])
                end_cue = Cue(break_end_pairs[i]["input_trigger"].split(":")[3])
...
def verifylog(body_log, config):
    #manual static item, for now, then may use body_log for loop for whole file
    for i in body_log:
        splitted = i.split(":")
    cue = Cue(splitted[3])
    uuid = splitted[2]
    #this loop is if one wants to attempt an general solution
    # for item in config.keys():
    #     if item not in ["recipient_emails", "frequency", "network_id"]:


    #This attempt is for specified solution for local break
    local_break_start = ""
    local_break_end = ""
    action_issues = []
    template_issues = []
    #timing_issues = []
    config_local_break = config["local_break"]
    if str(config_local_break).find("break_start"):
        local_break_start = config_local_break["local_break_start"]
    else:
        template_issues.append("Could not find local_break_start")
    if str(config_local_break).find("break_end"):
        local_break_end = config_local_break["local_break_end"]
    #else:
    
    
    lbs_input_trigger = local_break_start["input_trigger"]
    item_exist = False
    item_verify = False
    #loop through input_trigger of local_break_start
    for item in lbs_input_trigger.keys():
        if str(cue).find(item) > -1:
            item_exist = True
        else:
            item_exist = False
            template_issues.append(item)
            continue
        #if item exist, then check do they have the same value, splice_event_id is exception
        if lbs_input_trigger[item] == cue.get_command()[item] and item_exist:
            item_verify = True
        else:
            item_verify = False
            if item != "splice_event_id":
                action_issues.append(item)
        # check is the splice_event_id is numeric
        if type(cue.get_command()["splice_event_id"]) == int:
            item_verify = True
        else:
            item_verify = False
            action_issues.append("splice_event_id is not numeric")
    return [template_issues, action_issues, uuid]