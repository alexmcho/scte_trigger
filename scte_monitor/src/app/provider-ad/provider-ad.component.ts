...
@Component({
  selector: 'app-provider-ad',
  templateUrl: './provider-ad.component.html',
  styleUrls: ['./provider-ad.component.css'],
})
export class ProviderAdComponent implements OnInit {
  public index: number;
  public selfRef: ProviderAdComponent;
  ...
  setStartOutput() {
    try {
      if(this.providerAd.value[0].provider_ad_start.action == 'REPLACE') {
  ...
  setEndOutput() {
    try {
      if(this.providerAd.value[0].provider_ad_end.action == 'REPLACE') {
        this.endOutput = true;
      } else {
